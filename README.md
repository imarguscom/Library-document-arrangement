...
| Sheet | 用途 |
| --- | --- |
| `全部数据` | 单一来源时为所有完成字段映射的成果；多个来源时按 DOI 合并，但同 DOI 的期刊和会议记录分开保留 |
| `期刊论文` | Article 与 Review 类型记录；同时标记 Article 和 Proceedings Paper 时也归期刊，保留原始文献类型 |
| `会议论文` | 不含 Article/Review 标记的 Conference paper、Proceedings paper、workshop 等会议类记录 |
| `本校成果` | 本校成果模式或判定为本校的记录 |
| `校外成果` | 校外成果模式下已匹配邮箱、可进入认领流程的记录 |
| `待复核_可尝试原文补全` | 有 DOI 或 URL，且作者—单位关联、作者单位或通讯作者信息可尝试通过论文原文/出版社页面核验的记录 |
| `待复核_其他` | 无法通过原文直接补全的记录，例如学者匹配/邮箱待确认、别名冲突，或缺少 DOI/URL 的作者信息问题 |
...
3. 将不同来源字段映射到统一中文字段。
4. 单一来源时逐条完成字段映射，不以 DOI 去重；多个来源时标准化 DOI，并以 DOI 分组。若组内同时存在期刊与会议记录，则分别合并同类记录，类型不明/其他记录另组保留，避免它们把期刊和会议重新连在一起；无此分歧的 DOI 仍按原方式合并。
5. 合并来源库、收录类别、数据库记录号和引用指标。
6. 单条记录含 Article/Review 时归 `期刊论文`，即使同时标记 Proceedings Paper，也保留原始类型字符串；纯会议类记录归 `会议论文`。例如 WOS Article 与 Scopus Conference Paper 即使 DOI 相同也输出两条，各自保留字段。这两种类型情况不再单独触发待复核，作者、单位、邮箱等问题仍按原规则复核。
7. 根据本校/校外模式补充数据归属字段。
...
同一别名对应多个学者或邮箱时，系统不会自动裁决，会把记录放入 `待复核_其他`。
...
- 网页版的 `作品认领` 依赖别名表中“别名—邮箱”关系的完整性；输入候选邮箱可限定参与匹配的别名。

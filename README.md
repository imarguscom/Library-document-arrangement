...
| `待复核_其他` | 无法通过原文直接补全的记录，例如学者匹配/邮箱待确认、别名冲突、来源文献类型冲突，或缺少 DOI/URL 的作者信息问题 |
...
6. Conference paper、Proceedings paper、workshop 等会议类记录导出到 `会议论文`；若同一 DOI 同时被标记为会议类与 Article/Review，则转入 `待复核_其他` 并标注审核原因；`Review` 保留在期刊论文中。
7. 根据本校/校外模式补充数据归属字段。
...
- 这些补全仅使用本次输入文件，不请求外部接口，不建立跨次运行的核验知识库。文献类型冲突规则保持不变。
